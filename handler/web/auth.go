...
func (a *authWeb) Login(w http.ResponseWriter, r *http.Request) {
	// TODO: answer here
	login := path.Join("views", "auth", "login.html")
	header := path.Join("views", "general", "header.html")

	var tmpl = template.Must(template.ParseFS(a.embed, login, header))
...
func (a *authWeb) Register(w http.ResponseWriter, r *http.Request) {
	// TODO: answer here
	register := path.Join("views", "auth", "register.html")
	header := path.Join("views", "general", "header.html")

	var tmpl = template.Must(template.ParseFS(a.embed, register, header))
	exc := tmpl.Execute(w, nil)
	if exc != nil {		
		http.Error(w, exc.Error(), http.StatusInternalServerError)
		return
